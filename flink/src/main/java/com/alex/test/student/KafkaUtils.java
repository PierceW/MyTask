...
public class KafkaUtils {
    ...
    private static final String topic1 = "pub_visit_topic";  //kafka topic 需要和 flink 程序用同一个 topic
...
//        Producer<String, String> producer = new org.apache.kafka.clients.producer.KafkaProducer<>(props);
        ...
        try {
            for (int i = 1; i <= 10000; i++) {
                ...
                ProducerRecord record = new ProducerRecord<String, String>(topic1.concat("001"), null, null, JSON.toJSONString(jsonObject));
                producer.send(record);
                System.out.println("发送数据: " + JSON.toJSONString(JSON.toJSONString(jsonObject)));
            }
            Thread.sleep(3000);
        }catch (Exception e){
